...
V_M(T) = arr(V_Mo, Ea_VM, T)

# ╔═╡ 3438b410-3b23-43ab-85c1-10f0ea8db3d4
V_N(T) = arr(V_No, Ea_VN, T)

# ╔═╡ dc777f7c-872a-48e2-b3c3-4d56ecb5a811
K_G(T) = arr(K_Go, Ea_KG, T)

# ╔═╡ bc9dd7c9-d2bc-4824-b8fe-6a8fcd28c980
μ₁(T) = mm(V_G(T), K_G(T), G)

# ╔═╡ 14f09fb8-28c6-4233-b4cb-bff4eb21a0d6
K_M(T) = arr(K_Mo, Ea_KM, T)

# ╔═╡ 18e67836-ba95-4c03-ae93-2373c0f54c90
K_N(T) = arr(K_No, Ea_KN, T)

# ╔═╡ ac3343ca-c0da-4f2e-bc5c-88bfef08c114
K´_G(T) = arr(K´_Go, Ea_K´G, T)

# ╔═╡ 9fc25d16-1c05-4919-b391-268f2fc61823
μ₂(T) = mm(V_M(T), K_M(T), M)*lm(K´_G(T), G)

# ╔═╡ 5adbccc4-f6e9-4f02-8343-7b93d396c394
K´_M(T) = arr(K´_Mo, Ea_K´M, T)
...
function newsimul(u)
	prob.p[28] = u
	newsol = solve(prob, AutoVern7(Rodas4()))
	plot(newsol, idxs=[X E], label="new X + E")
	plot!(sol, idxs=[X E])
end
...
git-tree-sha1 = "b9fb76483ec33b1ff4b887be3a2e2c3f9687f16a"
